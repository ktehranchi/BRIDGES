...
foldername = "/Users/kamrantehranchi/Documents/GradSchool/Courses/Q5/Decision Making under Uncertainty/BRIDGES/Data"
...
#ApplianceProfilesLIQ2 = CSV.read("$(foldername)/ApplianceProfiles_LPG$(system)$(region).csv",DataFrame)
...
################################################################################
################################################################################
m = Model(optimizer_with_attributes(Gurobi.Optimizer,"Threads" => 8,"BarHomogeneous" => 1,"ScaleFlag"=>2, "FeasibilityTol"=> 0.005, "OptimalityTol" => 0.001, "BarConvTol"=> 0.0001, "Method"=> 2, "Crossover"=> 0))

if SetMIPGap == "Yes"
    set_optimizer_attribute(m, "MIPGap", 0.15)
end
...
# Appliance upgrades cost trigger per home
###############################################################################
HouseholdsNetwork = CSV.read("$(foldername)/Households$(system)$(num).csv",DataFrame)
HOUSEHOLDS = length(HouseholdsNetwork[:, :1])        # Number of household classes modeled
HouseholdTypes = HouseholdsNetwork[:,5]              # End-use services satisfied by each household class
upgrade_cost_HH = HouseholdsNetwork[:,7]             # Building infrastructure upgrade costs associated with electrifying this household [$]

@variable(m, 0 <= panelupgrade_APPS[I = 1:T_inv, a = 1:HOUSEHOLDS])
@variable(m, householdInfrastructureCosts[I = 1:T_inv, a = 1:HOUSEHOLDS] >= 0)
household_index = [[9 17 41]; [10 18 42]; [11 19 43]; [12 20 44]; [13 21 45]; [14 22 46]; [15 23 47]; [16 24 48]; [49 73 89]; [50 74 90]; [51 75 91]; [52 76 92]; [53 77 93]; [54 78 94]; [55 79 95]; [56 80 96]]
for an = 1:HOUSEHOLDS
    @constraint(m, [I = 1:T_inv, a = an], panelupgrade_APPS[I,a] >= sum(unitsbuilt_APPS[I,a0] for a0 = household_index[an,:])/3)
end
@constraint(m,[I = 1, a = 1:HOUSEHOLDS],  householdInfrastructureCosts[I,a] >= panelupgrade_APPS[I,a]*1000*upgrade_cost_HH[a])
...
#     global CostScenarios = CSV.read("$(foldername)/CostScenarios_High.csv",DataFrame)
# end
# if costscen =="LowCost"
#     global CostScenarios = CSV.read("$(foldername)/CostScenarios_Low.csv",DataFrame)
# end
